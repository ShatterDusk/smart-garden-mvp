...
class Container(ContainerModel):
    ...
    def start(self, interface, success_callback, **kwargs):
        ...
        if interface == "iothub" and iot_hub_helper is None:
            ...
            http_helper = HTTPHelper(api_url=None, container_id=self.id)
            ui.notify("HTTP API Verbindung wird hergestellt...", type="positive")
        ...
        self.thread = ContainerThread(target=self._thread_function, kwargs={"interface": interface, "iot_hub_helper": iot_hub_helper, "mqtt_helper": mqtt_helper, "http_helper": http_helper, "success_callback": success_callback})
        self.thread.start()
    ...
    def _thread_function(self, *args, **kwargs):
        ...
        http_helper = kwargs.get("http_helper")
        ...
        if interface == "iothub":
            ...
            for device in self.devices:
                device.start_simulation(interface, self._message_callback, http_helper=http_helper)
