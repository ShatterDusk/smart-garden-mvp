...
# 固定常量
UPLOAD_INTERVAL_MS = 60000  # 上报间隔：1分钟 = 60000毫秒（固定）
...
class HTTPHelper():
    ...
    def send_multi_sensor_data(self, device_name, plant_id, timestamp, sensor_data_dict):
        ...
        try:
            ...
            # 构造队列数据项（LocalTaskQueue.add 会包装成 {'recorded_at', 'data', 'created_at'}）
            queue_data = {
                'api_data': api_data,
    ...
    def _upload_loop(self):
        ...
        while self.running:
            ...
            # 等待下一次上报
            time.sleep(UPLOAD_INTERVAL_MS / 1000)
    ...
    def _upload_pending(self):
        ...
        for item in pending:
            try:
                # item 结构: {'recorded_at', 'data', 'created_at'}
                queue_data = item['data']
                api_data = queue_data['api_data']
                
                # 调试：打印上报数据
                logger.info(f"[DEBUG] 上报数据: {api_data}")
